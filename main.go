...
func appExec(port int, dbpath string) {
	...
	// TEST INSERT
	// TODO: remove
	statusRepository := status.Sqlite3Repository{}
	//statusRepository.Create(a.Db, status.Status{
	//	Service:  "new-service",
	//	Endpoint: "/test/euy",
	//	Status:   "OPEN",
	//})
	res := statusRepository.GetByServiceAndEndpoint(a.Db, "new-service", "/test/euy")
	for _, s := range res {
		fmt.Println(s)
	}
	// TEST INSERT END
